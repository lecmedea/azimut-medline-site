...
module.exports = async function chatHandler(request, response) {
  ...
  try {
    ...
    if (!answer) {
      ...
      return sendJson(response, 502, { error: "Филипп Филипович не смог подготовить ответ. Попробуйте ещё раз." });
    }

    return sendJson(response, 200, { answer });
  } catch (error) {
    console.error("AI chatbot backend error:", error);
    return sendJson(response, 500, { error: "Филипп Филипович временно недоступен. Попробуйте позже или позвоните в центр." });
  }
};
