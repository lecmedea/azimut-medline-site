(function () {
  ...
  const quickActions = [
    ...
    ["Написать в Telegram", "Подскажите, как написать в Telegram"],
  ...
  async function sendMessage(root, rawMessage) {
    ...
    try {
      ...
      history.push({
        role: "assistant",
        content: "Сейчас не получается получить ответ AI-помощника. Попробуйте позже или позвоните в центр: +7(905)27-277-03."
      });
      history = history.slice(-MAX_HISTORY);
      saveHistory();
      renderMessages(root);
